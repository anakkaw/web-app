        ...
        return (
            <div className="min-h-screen bg-stone-50 font-sans">
                ...
                <div className="flex flex-col items-center justify-center py-24 animate-page-enter">
                    <div className="inline-flex items-center justify-center p-5 bg-white border border-stone-200 shadow-sm rounded-full mb-6">
                    ...
                    <Link href="/" className="inline-flex items-center gap-2 px-6 py-2.5 bg-stone-900 text-white rounded-xl font-bold text-sm hover:bg-black transition-all shadow-lg shadow-stone-900/10">
            ...
            <div className="container mx-auto px-4 py-8 max-w-5xl animate-page-enter">
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
                    <div className="space-y-2 flex-1">
                        <div className="flex items-center gap-2 text-stone-500 font-bold text-xs mb-2 uppercase tracking-wide">
                            ...
                            <div className="flex-1 min-w-[200px]">
                                {isEditingTitle ? (
                                    <Input
                                        ...
                                        className="text-2xl md:text-4xl font-black tracking-tight h-auto py-2 px-4 border-orange-300 focus:ring-orange-500 bg-white shadow-lg w-full rounded-xl"
                                    />
                                ) : (
                                    <h1
                                        onClick={() => !isReadOnly && setIsEditingTitle(true)}
                                        className={`text-2xl md:text-3xl lg:text-4xl font-black tracking-tight text-stone-900 leading-tight ${!isReadOnly ? 'cursor-pointer hover:opacity-80' : ''} transition-all flex items-center gap-3 select-none group`}
                                    >
                                        {formData.name || "ชื่อโครงการ..."}
                                        {!isReadOnly && (
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-stone-300 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" /><path d="m15 5 4 4" /></svg>
                                        )}
                                    </h1>
                                )}
                            </div>
                            ...
                            <div className="flex items-center">
                                {isEditingCode ? (
                                    <Input
                                        ...
                                        className="w-32 text-sm font-bold bg-white shadow-lg border-orange-300 focus:ring-orange-500 rounded-lg"
                                    />
                                ) : (
                                    <div
                                        onClick={() => !isReadOnly && setIsEditingCode(true)}
                                        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white border border-stone-200 shadow-sm ${!isReadOnly ? 'hover:border-orange-300 hover:shadow-md cursor-pointer' : ''} transition-all group`}
                                    >
                                        <span className="text-xs font-bold text-stone-500 uppercase">Code</span>
                                        <div className="flex items-center gap-1">
                                            <span className="text-sm font-bold text-stone-900 group-hover:text-orange-600">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    {/* Metric Cards */}
                    <Card className="relative overflow-hidden border-none shadow-md rounded-xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white transform hover:-translate-y-1 transition-all duration-300">
                    ...
                    <Card className={`relative overflow-hidden border-none shadow-md rounded-xl text-white transform hover:-translate-y-1 transition-all duration-300
                         ${formData.progressLevel === 'Done' ? 'bg-gradient-to-br from-emerald-500 to-teal-700' :
                            formData.progressLevel === 'In Progress' ? 'bg-gradient-to-br from-orange-500 to-amber-600' :
                    ...
                    <Card className={`relative overflow-hidden border-none shadow-md rounded-xl transform hover:-translate-y-1 transition-all duration-300 ${categoryColors.bg} text-white`}>
                        <div className="absolute top-0 right-0 p-3 opacity-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    {/* Owner Card */}
                    <Card className="relative overflow-hidden bg-white border border-stone-200 shadow-sm rounded-xl group hover:shadow-md transition-all duration-300">
                        ...
                        <CardContent className="px-5 pb-4">
                            <Input
                                ...
                                readOnly={isReadOnly}
                                className={`h-11 text-lg font-bold border-stone-200 focus:border-stone-800 focus:ring-stone-900/5 rounded-lg ${isReadOnly ? 'bg-transparent border-transparent' : 'bg-white'} transition-all`}
                    ...
                    <Card className="relative overflow-hidden bg-white border border-stone-200 shadow-sm rounded-xl group hover:shadow-md transition-all duration-300">
                        ...
                        <CardContent className="px-5 pb-4">
                            {formData.progressLevel === 'Not Start' ? (
                                <div className="h-11 flex items-center text-lg font-medium text-stone-400 italic">
                                    ยังไม่กำหนด
                                </div>
                            ) : (
                                <Input
